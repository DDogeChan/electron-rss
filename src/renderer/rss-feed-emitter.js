...
class RssFeedEmitter extends TinyEmitter {
  ...
  _validateFeedObject(feed) {
    ...
    if (!feed.url || typeof feed.url !== "string" || feed.url === "") {
      throw {
        type: "type_error",
        message:
          'Your configuration object should have an "url" key with a string value'
      };
    }
  ...
  _createSetInterval(feed) {
    ...
    function getContent() {
      ...
      function redefineItemHistoryMaxLength(data) {
        ...
        data.feed.maxHistoryLength =
          feedLength * instance._historyLengthMultiplier;
      }
  ...
  _test(data) {
    if (data.newItems.length) this.emit("new-data", data);
  }
